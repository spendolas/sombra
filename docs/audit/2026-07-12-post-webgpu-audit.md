...
**Fix-session addendum 8 (same day, commit `60cac5b`):** WebGPU >8-intermediates breakage fixed — the 8-cap wasn't a GPU constraint (WebGL FBO pool is uncapped); it caused per-frame texture-pool thrash (resize check compared uncapped count), a render loop that broke before the final canvas pass, and silently-null bind groups. Ceiling now 32 as a loud memory guard: over-cap plans are rejected by `updateMultiPass` with an actionable message, and `applyCompileResult` now pushes renderer plan rejections into compilerStore/banner (first slice of renderer-error surfacing). Live-verified: 11-pass chain renders with a frame-stable 10-texture pool; 33-deep chain banners "max 32 — reduce effect chain depth".

**Fix-session addendum 9 (2026-07-13, commit `9174e74`):** anchor 3×3 toggle grid shipped (user-requested UX; user delegated the design). Full golden-rule pipeline both directions: DB component `717:15` → `ds.anchorGrid.*` → `AnchorGrid` in NodeParameters (opt-in via new `NodeParameter.control: 'anchor-grid'`; enum values/compile machinery untouched), AND Figma component built via grip (Anchor Cell variant set + Anchor Grid molecule, fully variable-bound) with node ids backfilled into the DB; wiki page + index added. Live-verified on both node card and properties panel, plus Figma geometry/PNG. Closes the "User-requested UX changes" item.

**Fix-session addendum 10 (2026-07-13, commit `892e027`) — SWEEP COMPLETE:** tooling hygiene shipped — eslint config (ignores + `_`-prefix) then every surviving problem fixed, not suppressed: repo lints at **zero** (was 38, incl. a stale `.claude/worktrees` double-count and a real CommandPalette stale-`mousePosition` bug); BROWSER-AUTOMATION.md node tables regenerated from the registry (41 nodes, current SRT params) + port/coercion table + new API docs (shareGraph, renderer instance, store actions, `?backend=webgl2`) → **self-validate 0 FAIL / 0 WARN**; drift-check skips synthetic code-only DB keys. Outstanding user-side items: renew `FIGMA_TOKEN` (REST 403; blocks `drift:collect`/`tokens:sync` — grip write path unaffected) and the gated **push** (26+ unpushed commits → GH Pages deploy). Every actionable audit item in the fix-order list is now closed; remaining §Testing-gaps harnesses (cross-backend pixel-diff, image-loaded WGSL fixture in suite) are future work.

**Fix-session addendum 11 (2026-07-13, commit `7c8b949`):** `figma-audit.ts` migrated to the Grip bridge — the last REST-only DS script, dead since the FIGMA_TOKEN expired. A plugin-side `run_script` serializes each audited node subtree into REST `/nodes` shape (lineHeight/letterSpacing→px, paint-level variable bindings lifted to `boundVariables.fills/strokes`, `dashPattern`→`strokeDashes`, `figma.mixed` guarded, `openTypeFeatures`→`opentypeFlags`), so extraction/diff/`--fix`/`--strict` are untouched; `--rest` forces the old path. The "renew FIGMA_TOKEN" item from addendum 10 is now moot — every DS script (pull, drift-collect, audit) runs tokenless via Grip; REST is optional fallback only. Live grip run: 67/68 parts audited (68th = dead `textGhostButton` 326:36, known DS debt), 31 actionable diffs surfaced — all real Figma↔DB drift (anchorGrid layout fields, button hover fills rebound to `highlight`, per-side nodeCard strokes the REST field-mapping was blind to, unregistered `white` variable) — queued for the DS-debt planning pass, deliberately not auto-fixed here.

**Fix-session addendum 12 (2026-07-13, commits `b45e491` + `1b0e787`) — DS DEBT CLOSED:** the 31 diffs from addendum 11 are resolved to **audit zero** (0 mismatch / 0 missing / 0 extra over 72 parts) and drift-check reports **no component drift** (was 4 figma-only + 1 db-only). Breakdown: (a) real drift adopted from Figma via `--fix` — button solidHover/ghostHover/textGhostHover fills → `interactive/highlight`, anchorGrid cell/cellActive gain typed layout/justify/align replacing hand-written `extra`; (b) audit false positives killed in code — dbProps now translates the stroke `side` shorthand (`{side:'bottom'}` → `stroke.bottomWeight`) instead of emitting an all-side weight, un-flagging 4 nodeCard parts; (c) intentional code divergences annotated with `auditIgnore` (fluid widths on nodeCard.content/gradientEditor/sliderTrack, anchorGrid CSS-grid-vs-autolayout); (d) registry gaps closed — `white` (VariableID:106:16) registered → `--white`/`--color-white`, sliderTrack height moved to its typed field; (e) all 4 orphan Figma components registered: Image node template 551:977 (templates now cover all 41 nodes), Node Preview 619:1950 → `ds.nodePreview.root` (wired into ShaderNode), Image Viewport Overlay 556:1144 → `ds.imageViewportOverlay.root` (wired into ImageUploader, `overflow:hidden` typed), Cursor Icon set 569:201 (tracking-only — cursors are runtime SVG data-URIs); (f) dead `textGhostButton` re-keyed 326:36 → synthetic `tgb:1` (Figma node deleted by design; code still uses the entry). tokens:check, schema, tsc, eslint, live smoke all green.
**Fix-session addendum 13 (2026-07-14):** WebGL preview parity for empty image nodes — found during a manual WebGL2 eyeball pass. An image node with no image loaded showed a **gray** mini-preview under WebGL2 while WebGPU showed none. Root cause: an empty image node's shader outputs a `vec3(0.5)` gray placeholder and never samples its (still-declared) `u_*_image` uniform. WebGPU's preview bails to `null` on the unfilled texture binding → no thumbnail; WebGL happily renders the valid gray-placeholder shader → gray thumbnail. First attempt detected image samplers via `gl.ACTIVE_UNIFORMS` and failed silently: the driver strips the declared-but-unused sampler from the active list, so the bail never fired (an earlier "no thumbnail" screenshot was a pre-render frame misread as success). Corrected: `hasMissingImageTexture(fragmentShader)` scans the shader **source** for a declared `uniform sampler2D u_*_image` absent from `imageTextures` (source catches the unused declaration that `ACTIVE_UNIFORMS` drops; `_image` uniquely identifies image nodes — pass samplers are `u_passN_tex`). Bail in both single- and multi-pass paths, mirroring WebGPU; `invalidateNode()` re-renders once the texture uploads. Live-verified on `?backend=webgl2` (both states screenshotted): empty image node → no thumbnail (matches WebGPU, confirmed side-by-side); after upload → thumbnail renders. tsc + eslint clean; IR parity 46/46, WGSL multipass 159/159. Closes the last known WebGL-preview divergence.
