...
export function assembleWGSL(
  ...
  // 7. Emit sombra_mod helpers (WGSL has no mod() and cannot overload a user fn
  // by signature). Each overload has a name-per-type, and the backend emits the
  // typed name at lowering (sombra_mod / _v2 / _v3 / _v4), so emit exactly the
  // helper whose CALL NAME actually appears — deterministic, no type-guessing
  ...
  const modVariants: Array<[RegExp, string]> = [
    ...
    [/\bsombra_mod_v3\(/, 'fn sombra_mod_v3(x: vec3f, y: vec3f) -> vec3f { return x - y * floor(x / y); }'],
    [/\bsombra_mod_v4\(/, 'fn sombra_mod_v4(x: vec4f, y: vec4f) -> vec4f { return x - y * floor(x / y); }'],
  ]
  const emitted = modVariants.filter(([re]) => re.test(allCode)).map(([, def]) => def)
  const modHelpers = emitted.length ? emitted.join('\n') + '\n\n' : ''
