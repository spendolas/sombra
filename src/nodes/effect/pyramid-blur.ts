/**
 * Pyramid Blur — a RADIUS-ADAPTIVE PYRAMID Gaussian in linear light with
 * premultiplied alpha and a dithered write. A SEPARATE node from the default
 * `blur` (Gaussian Blur): this is the heavy, large-STATIC-radius variant.
 *
 * Use the default `blur` node when radius must be wired or animated — that is the
 * app's normal shape and stays a linear-sampled separable Gaussian with a
 * connectable, uniform-updated radius. This pyramid node deliberately trades that
 * away for radius-independent cost (see the TRADE-OFF note below); it exists for
 * screen-spanning static blur where the separable path gets expensive.
 *
 ...
 * needing a wireable/animatable radius uses the default `blur` node (Gaussian Blur), the
 * linear-sampled separable Gaussian the bake-off names as the fallback. Making THIS node
 * animatable is under investigation (a fixed max-N compile with uniform-driven active
 * passes + coarse σ).
 */
...
export const pyramidBlurNode: NodeDefinition = {
  type: 'pyramid_blur',
  label: 'Pyramid Blur',
  category: 'Effect',
  description: 'Heavy radius-adaptive pyramid Gaussian for large STATIC blur (radius is compile-time, not animatable — use Gaussian Blur to wire/animate radius)',
