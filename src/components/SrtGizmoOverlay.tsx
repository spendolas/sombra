...
export function SrtGizmoOverlay({ dockTargetRef, floatTargetRef, fullTargetRef }: SrtGizmoOverlayProps) {
  ...
  // 1:1 CSS px (shader: vec2(tx,-ty)·u_dpr/u_resolution) with SCREEN parity
  // (+ty down) → coordSpace 'screen_px' (see resolveSRT; directions user-verified
  // by dragging). Previously filed 'auto_uv', which matches only at dpr=1 (the
  // original measuring display) — on retina the gizmo lagged the ribs by dpr.
  // Still node-frame translate storage → 'node-legacy' (compensates rotate/scale
  // drags so the centre doesn't orbit).
  ...
  const translateFrame: 'world' | 'node-legacy' = spatial ? 'world' : (dialect?.translateFrame ?? 'world')
  const effectiveCoordSpace: SRTCoordSpace = spatial ? coordSpace : (dialect?.coordSpace ?? coordSpace)
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      const r: ResolvedSRT = resolveSRT(latestParams, {
        transforms,
        coordSpace: effectiveCoordSpace,
        translateFrame,
  ...
  }, [dragging, selectedNode, transforms, translateFrame, effectiveCoordSpace, outputAnchor, updateNodeData])
