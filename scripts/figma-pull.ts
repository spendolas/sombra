 ...
 *   1. REST API /variables/local (requires Enterprise + file_variables:read scope)
 *   2. Plugin API cache file (tokens/.figma-vars-cache.json)
 *
 * The cache file is generated by running the Figma Plugin API snippet
 * (see scripts/figma-dump-vars.js) in the Figma dev console or via
 * Claude's Chrome extension. Run `npm run tokens:dump` to see instructions.
 *
...
async function main() {
  ...
  // Step 2: Fetch variables (requires Enterprise plan)
  console.log('Fetching variables...')
  const varsData = await figmaTryGet(`/files/${fileKey}/variables/local`) as null | {
    meta: {
