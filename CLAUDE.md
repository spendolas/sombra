...
Phases 0–5 complete: editor, compiler, 44 nodes, save/load `.sombra` files, compact share URLs, per-node mini-previews, Cmd+K palette, design-system pipeline. **WebGPU migration complete** (main + preview renderers; WebGL2 fallback retained). Recent work: **Phase 6 multi-pass composable effects** (`PHASE6-MULTIPASS.md`) — relay passes for fragColor conflicts, ping-pong aliasing fixes for deep pass chains, 9-point anchor pin on Fragment Output. Sprint-by-sprint history lives in `ROADMAP.md`.
