...
- **`raw()` budget.** Two-arg `raw(glsl, wgsl)` is hand-written per backend and drifts by construction — it stands at **12**; freeze there and ratchet down. One-arg `raw()` is for whole shared helper-function bodies only, never node body logic.
